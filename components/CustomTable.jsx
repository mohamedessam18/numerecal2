...
const CustomTable = (props) => {
  ...
  const customizeValue = (header, value, row) => {
    ...
    // if header is i
    if (header === 'i') return <td className={Styles.td + ' ' + Styles.i_col}>{displayValue}</td>;
    // if row is first row put ea as '-'
    else if (header === 'ea' && row === 0) return <td className={Styles.td}>{'-'}</td>;
    // if column is ea
    else if (header === 'ea') return <td className={Styles.td}>{displayValue + '%'}</td>;
    ...
    // if any other column
    else return <td className={Styles.td}>{displayValue}</td>;
  };
