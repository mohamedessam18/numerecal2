...
export default function XProvider({ children }) {
  ...
  React.useEffect(() => {
    ...
    if (settings) {
      ...
      setDecimalPoints(settings.decimalPrecision?.decimalPlaces ?? 3);
      setWithRounding(settings.decimalPrecision?.withRounding ?? false);
    } else {
      const settingsData = {
        decimalPrecision: {
          decimalPlaces: 3,
      ...
      setDecimalPoints(settingsData.decimalPrecision.decimalPlaces);
      setWithRounding(settingsData.decimalPrecision.withRounding);
  ...
  const calculate = ({ name, operation, values, validationData, setShowSolution, setData, executeScroll }) => {
    ...
    try {
      ...
      showMsg('warning', error.message || 'Could not solve this input.');
      setShowSolution(false);
      return;
    }
