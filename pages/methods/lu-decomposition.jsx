...
const LuDecomposition = () => {
  ...
  React.useEffect(() => {
    if (router.query.operation === 'calculateQuery' && formRef.current.sol_1.value === '') {
      ...
      values.matrix.forEach((row, rowIndex) => {
        formRef.current[`x1_${rowIndex + 1}`].value = row[0];
        formRef.current[`x2_${rowIndex + 1}`].value = row[1];
        formRef.current[`x3_${rowIndex + 1}`].value = row[2];
        formRef.current[`sol_${rowIndex + 1}`].value = row[3];
      });
      if (formRef.current.withPP) formRef.current.withPP.checked = values.withPP;
